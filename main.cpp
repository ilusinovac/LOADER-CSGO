				...
				ImGui::BeginChild(xorstr_("##main"), ImVec2(g_globals.client_side.window_settings.width - 14.f, g_globals.client_side.window_settings.height - 32.f), false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoSavedSettings);
				{
					...
					if (activation_tab)
					{
						...
						if (activation_success == true)
						{
							...
							if (notifydone) 
								activation_validted = true, activation_tab = false, activation_success = false;
						}
					}
					if (activation_validted == true)
					{
						static bool cs_go = false;
						...
						for (int i = 0; i < (int)split_cheat_structure.size(); i++)
						{
							...
							{
								if (split_cheat_structure[0] == xorstr_("csgo"))
								{
									...
									else if (split_cheat_structure[1] == xorstr_("Detected"))
									{
										li(MessageBoxA)(NULL, xorstr_("Cheat is currently detected, sorry for the inconvenience"), xorstr_("Loader"), MB_ICONERROR);
										break;
									}
						...
						if (cs_go == true)
						{
							//injector::inject(csgo, sizeof(csgo));
						}
					}
				}
				ImGui::EndChild();
